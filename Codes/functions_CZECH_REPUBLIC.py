...
dict_brand = dict({'nike':'&brand_id[]=10', 'adidas': '&brand_id[]=46', 'zara':'&brand_id[]=260', "levi's":'&brand_id[]=1026',\
'h&m':'&brand_id[]=5', 'ralph lauren':'&brand_id[]=493', 'mango':'&brand_id[]=16', 'lacoste':'&brand_id[]=97', \
'calvin klein':'&brand_id[]=38', 'tommy hilfiger': '&brand_id[]=94', 'guess':'&brand_id[]=109', 'michael kors':'&brand_id[]=1615', \
'jordan':'&brand_id[]=5285', 'puma':'&brand_id[]=18', 'balenciaga':'&brand_id[]=3230', 'vans':'&brand_id[]=497'})
...
def research_and_scrape(query, criteria_string, end_page = 2):
    ...
    for page in range(2, end_page + 1):
        print('We start to scrape page n° {}'.format(page))
        search_url = 'https://www.vinted.cz/predmety?search_text=' + query.replace(' ', '%20') + criteria_string + '&page={}'.format(page) # 
        DATA_ALL = DATA_ALL.append(scrape_research_page(search_url))
        print('We finished to scrape page n° {}'.format(page))
...
def collect_info(page_url):
    ...
    try:
        price = re.split(' Kč', clean_data[0])[0]
    except:
        price = 'Na'

    try:
        brand = clean_data[np.where([(y[:6]=='ZNAČKA') for y in clean_data])[0][0]][6:].strip() # obtain brand without any spaces
    except:
        brand = 'Na'

    try:
        colors = clean_data[np.where([(y[:5]=='BARVA') for y in clean_data])[0][0]][5:].strip() 
    except:
        colors = 'Na'

    try:
        size = clean_data[np.where([(y[:7]=='VELIKOST') for y in clean_data])[0][0]][7:].strip() 
    except:
        size = 'Na'

    try:
        condition = clean_data[np.where([(y[:4]=='STAV') for y in clean_data])[0][0]][4:].strip() 
    except:
        condition = 'Na'

    try:
        views = clean_data[np.where([(y[:15]=='POČET ZHLÉDNUTÍ') for y in clean_data])[0][0]][15:].strip() 
    except:
        views = '0'

    try:
        interested = re.split(' ',clean_data[np.where([(y[:7]=='ZÁJEMCI') for y in clean_data])[0][0]][7:].strip())[0]
    except:
        interested = '0'

    try:
        location = clean_data[np.where([(y[:6]=='LOKACE') for y in clean_data])[0][0]][6:].strip() 
    except:
        location = 'Na'
    ...
    try:
        description = clean_data[np.where([(y[:7]=='NAHRÁNO') for y in clean_data])[0][0] + 1].strip() 
    except:
        description = 'Na'
...
def interactive_version():
    ...
    query = input("Which item do you want to research on vinted.cz ? \n")
